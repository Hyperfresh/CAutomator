...
module.exports = {
    ...
    callback: (message) => {
        ...
        let embed = new Discord.MessageEmbed()
            ...
            .addField('Operating System',`> ${os.platform()} ${os.release()} ${os.arch()}\n*(${os.version()})*\n**CPU**: ${os.cpus()[0].model}\n**Total memory**: ${(os.freemem()/1e+9).toFixed(2)}GB/${(os.totalmem()/1e+9).toFixed(2)}GB\n(${((os.totalmem()/os.freemem())*100).toFixed(2)}% used, ${((os.freemem()/os.totalmem())*100).toFixed(2)}% free)\n**Uptime**: ${ut_hour}h${ut_min}m${ut_sec}s`,true)
            .addField('Node Installation',`> ${process.version}\n**Memory heap**: ${((process.memoryUsage().heapUsed)/1e+6).toFixed(2)}MB/${((process.memoryUsage().heapTotal)/1e+6).toFixed(2)}MB (${((process.memoryUsage().heapTotal/process.memoryUsage().heapUsed)*100).toFixed(2)}% used, ${((process.memoryUsage().heapUsed/process.memoryUsage().heapTotal)*100).toFixed(2)}% free)`,true)
